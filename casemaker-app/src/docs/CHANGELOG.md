...
- **One screw-hole mechanism, and modelled printed threads** ([#140](https://github.com/SlyWombat/CaseMaker/issues/140)): every screw hole now comes from `engine/compiler/fasteners.ts` — a sourced M2–M6 table (ISO 261 / 68-1 / 273 / 4762 / 7380-1 / 10642 / 14583) plus `screwHole()`, `screwStarter()` and `threadTool()`. Head recess, clearance hole and receiving hole are three different things sized three different ways, and the table keeps **two** pilot columns because a 60° machine screw and a thread-forming screw want holes nearly a millimetre apart — collapsing them is what produced the original defect. `screwStarter({ mode: 'pre-threaded' })` cuts a real helical thread, gated on **pitch** rather than size: below about two nozzle widths the female crest does not print and the thread comes out a smooth bore at the major diameter, so M5 and M6 qualify and M4 down quietly falls back to self-tap. `rack.ts` is migrated with its geometry unchanged. New `npm run thread:coupon` prints a fit ladder; `npm run pilot:coupon` now takes a size.
- **Fan-over-screw-column warning**: a fan opening is a 36–56 mm hole, so parking one on an accessory screw column deletes that fixing point for the slots it covers — geometry cannot avoid it. The placement banner now names the column, lists the affected slots, and says how far to move the fan to clear it.

- **Synthesised board placeholder in 3D viewport** (`engine/scene/boardPlaceholder.ts`): every board now renders a green PCB plus mounting-hole rings plus per-component blocks (coloured by `kind`) inside the case cavity, derived directly from `BoardProfile`. No GLB asset required — the placeholder is the always-available baseline for boards that lack a photo / 3D model. The case shell + lid materials drop to ~55–60 % opacity so the board is visible through them.
- **CAD reference repos** under `samples/cad-references/`: Raspberry Pi HAT mechanical drawings (`raspberrypi/hats`) and a KiCad B+ HAT template (`devbisme/RPi_Hat_Template`) for cross-checking HAT JSON dimensions.
- **Per-board CAD reference library** ([#40](https://github.com/SlyWombat/case-maker/issues/40)): one subdirectory per built-in board under `samples/cad-references/boards/<board-id>/` with mechanical PDFs, schematics, and (where available) full PCB source. Pulled from official manufacturer pages where redistribution is permitted (Arduino docs, Raspberry Pi datasheets, BeagleBoard, M5Stack, micro:bit Foundation, PJRC) and community KiCad libs where the manufacturer doesn't publish CAD (DOIT ESP32 DevKit V1). Each subdir carries a `NOTES.md` with the upstream URL + license. Wired `crossReference` into all 12 board JSONs. Jetson Nano + Teensy 4.1 are NOTES-only because the upstream sources are account-gated or have never been released.
- **Procedural fixture library** ([#39](https://github.com/SlyWombat/case-maker/issues/39) Phase 1): the placeholder now upgrades each component to a recognisable shape — USB ports with recessed throats, RJ45 with shielded body + latch slot, GPIO headers tiled with 2.54 mm pins, FPC connectors with flap, fan-mount frame + hub, UFL antenna, plus per-`fixtureId` builders for `xlr-3` and `audio-jack-3-5`. Tagged the DMX shield XLRs and the GIGA audio jack with the new `fixtureId` field.
- **Extra cavity height** ([#36](https://github.com/SlyWombat/case-maker/issues/36)): new `caseParameters.extraCavityZ` knob (slider in the case-parameters panel, 0–100 mm). Adds purely additive headroom — wall and lid grow by exactly that amount, port and HAT cutouts keep their absolute Z so connectors stay aligned.
- **Clone-board metadata editor** ([#38](https://github.com/SlyWombat/case-maker/issues/38)): the existing `cloneBoardForEditing` action now ships with a metadata editor in the board panel — name, manufacturer, default standoff height, recommended Z-clearance, cross-reference URL, datasheet revision. Component editor also exposes `cutoutShape` and `fixtureId`.
- **Placement validator** ([#37](https://github.com/SlyWombat/case-maker/issues/37)): new `engine/compiler/placementValidator.ts` runs as part of `compileProject` and attaches a `PlacementReport` to the BuildPlan. Catches per-face cutout overlaps, off-PCB mounting holes, mounting-hole crowding, off-PCB component centroids, HAT incompatibility, HAT stack collisions, and rim margin violations. A new `PlacementBanner` shows errors (red) and warnings (yellow) in the corner of the viewport.

### Fixed

- **Rack floor ribs bridged the cable notches** ([#142](https://github.com/SlyWombat/CaseMaker/issues/142)): the bottom plate's stiffening ribs hang below the deck's outer face, and the perimeter rib runs along the very edge the notches open through — so a cut only `PLATE_T` deep left a 3 mm bar across the mouth of every notch, with the cross ribs closing anything deeper than the 45 mm pitch. The notches now cut the full section. The two features had only ever been tested apart: the notch case used the 252 mm sample, where the span is under the 260 mm rib threshold, and the rib case set no notches.
- **Rack floor ribs blocked the tab screws' driver paths**: the bottom plate's tab screws are driven UP from the rack's underside, so a driver reaches each head counterbore through the same face the ribs hang off — 18.5 mm³ of perimeter rib stood in every one of those four Ø10.8 access circles. The ribs are notched clear of them.
- **Rack mid bearing pad widened to 46 mm** ([#143](https://github.com/SlyWombat/CaseMaker/issues/143)): the two end tabs are each pulled onto their stacking foot by an M5, but the mid tab has no screw at all, so bearing is its whole fixing. At the end-foot length of 30 mm the pad ran out only 4 mm past each end of the 22 mm tab; it now leaves 12 mm.
- **Save as… and Load threw "Illegal invocation"** ([#144](https://github.com/SlyWombat/CaseMaker/issues/144)): the File System Access pickers were handed back as bare function references and called as methods of a plain object, so `this` was not `window` and Chrome's binding refused. Export was unaffected, which is why STL export worked and Save as… did not.
- **Rack fan mounts no longer close the vent windows** — a fan was carried on a full-thickness strip spanning the panel, and even lightened the strip left `POCKET_SKIN` across every window it crossed, so the panel came out closed exactly where air was meant to move. With a fan on each side (one pushing, one pulling) the windows are the intake and exhaust path. The mount is now a pad carrying the bolt circle, hung off the nearest structural members by two rails. Measured on full-thickness line-of-sight probes: the strip closed 30 % of the side panel's open area at sample size (16 % on a tall rack), the pad and rails close 9 % (6 %) — most of which is the pad the fan bolts to. Material added per fan drops to 44.2 cm³ and is now the same on every rack shape (was 62.3–100.8 cm³).

- **Rack fan strip no longer thins the screw bosses** — the lightening pockets inside a fan strip are blind cuts from the outer face, and nothing kept them clear of the accessory screw columns they crossed. Every column inside the fan band lost ~70 % of its boss (821 → 244 mm³) and was then re-drilled, so the hole looked right and bit into ~1 mm of skin. The pockets now carry the same boss keep-outs the panel's own lightening uses. This was the reported "crowding the screw posts".

- **Rack side fans: mirrored panel got no cutouts** — the fan cuts computed their own lateral start as `xr(-OVER, 0)`, which mirrors to `[width, width + 1]`. On the right-hand panel every cutting cylinder therefore began on the outer face and ran away from the part, so adding one fan per side produced one working panel and one solid wall. The cuts now share `holeX`/`holeLen` with the screw columns, which mirror correctly.
- **Rack fan mount no longer spans the full height** — the mounting strip always ran rail to rail, so a tall rack paid its entire height in solid material for a local mount and swept a whole screw column into the strip on the way past. The strip now takes whichever span is shorter, height or depth: fan cost on a 28-slot rack drops from 100.8 cm³ to 62.3 cm³ and stops tracking rack height at all. Squat, deep racks still get the vertical strip. A full-depth strip crosses the rear anchor column, so that column is now re-drilled through the strip alongside the front and mid ones.

- **Smart-cutout pre-pass standoff omission** ([#43](https://github.com/SlyWombat/case-maker/issues/43)): `engine/compiler/smartCutoutLayout.ts:44` was computing `cutoutTopWorldZ` without `defaultStandoffHeight`, so the bridge-thinning check never fired after issue #28 propagated standoff into `ports.ts:50`. Smart-layout now matches the actual cutout Z. Issue #17's printability check works again.
- **Screw-down validator standoff omission** ([#44](https://github.com/SlyWombat/case-maker/issues/44)): `engine/compiler/validation.ts:42` was computing `componentTopZ` without standoff while `boardTopZ` included it, biasing every collision check by exactly one standoff. Real `+z` component / lid-post collisions now flag.
- **VESA mount holes pierce non-`-z` faces** ([#45](https://github.com/SlyWombat/case-maker/issues/45)): `engine/compiler/mountingFeatures.ts::generateVesaMount` was emitting Z-axis cylinders for every face. New `axisCylinder(facing, length, radius, segments)` primitive in `engine/compiler/buildPlan.ts` orients the through-hole along the face normal; VESA holes on `+y`/`+x`/`-x` faces now actually pierce the wall.
- **`computeShellDims` HAT-blind callers** ([#46](https://github.com/SlyWombat/case-maker/issues/46)): every consumer (`lid.ts`, `snapCatches.ts`, `displays.ts`, `antennas.ts`, `ventilation.ts`) now threads `hats` + `resolveHat` through to `computeShellDims`. The signature drops its optional defaults so the type system catches future omissions. `coords.ts` gains `pcbTopZ`/`pcbBottomZ`/`cavityOriginXY` helpers as the canonical Z-formula source.
- **Tauri CSP no longer blocks LAN-bound origin** ([#47](https://github.com/SlyWombat/case-maker/issues/47)): the embedded HTTP server now sends a `Content-Security-Policy` header on every response that uses `'self'` instead of hard-coded loopback hostnames; `tauri.conf.json` mirrors the same policy. The `--host=<LAN IP>` flag and the installer's `/HOST=` option now produce a working SPA, not just a working bind.
- **Removed `'sliding'` joint references** ([#48](https://github.com/SlyWombat/case-maker/issues/48)): `CasePanel.tsx` joint dropdown, `lidPostsAcrossJoints.spec.ts`, and the e2e `'sliding lid'` test now reflect the three-joint enum (`flat-lid`, `snap-fit`, `screw-down`). E2e test rewritten to assert the schema's preprocess-coercion of removed values.

### Added

- **Features panel** ([#49](https://github.com/SlyWombat/case-maker/issues/49)): new `FeaturesPanel.tsx` in the sidebar exposing the six engine domains that previously had no UI: display selector + framing toggle, antennas (add/enable/type/facing/remove), fan mounts (size/face/grille), text labels (text/face), mounting features (4-corner-tabs / VESA presets, enable, remove), snap catches (per-wall list with uPosition, only shown when joint is snap-fit), and a recessed-lid checkbox. Adds `addAntenna`/`removeAntenna`/`patchAntenna` and `addSnapCatch`/`removeSnapCatch`/`patchSnapCatch` store actions.

- **GIGA + DMX template real-world validation** ([#35](https://github.com/SlyWombat/case-maker/issues/35)): the GIGA R1 board profile carried a fictional USB-B "programming" port and DC barrel jack (the real Giga has neither — it programs over USB-C and powers via USB-C / USB-A / VIN). Replaced with the actual 3.5mm audio jack and the right-edge MIPI camera + DSI display flex connectors. The DMX template (and the Uno DMX template) was also pushing HAT placements with `ports: []`, which silently dropped the XLR cutouts; both now auto-fill via `autoPortsForHat`. The Giga template additionally applies a Mega-form-factor x-offset so the Uno-form-factor shield sits over the host header column, not in free air.
